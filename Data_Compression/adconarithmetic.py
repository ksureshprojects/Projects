...
def enfreq(t):
    p = [[(i, t[k][i]/t[k][128]) for i in range(128)] for k in range(129)]
...
def enupdate(p,f,t,c):
    p[c] = [(i, t[c][i]/t[c][128]) for i in range(128)]
    f[c] = []
    f[c].append((p[c][0][0],0)) #First conditional cumalative probability term (symbol, value)
    for k in range(1,len(p[c])):
        f[c].append((p[c][k][0],f[c][-1][1] + p[c][k-1][1]))
        
    f[c] = dict(f[c])
    p[c] = dict(p[c])
    return p,f

def deupdate(p,f,t,c):
    p[c] = [(i, t[c][i]/t[c][128]) for i in range(128)]
    f[c] = []
    f[c].append([p[c][0][0]])  #(Symbol)
    f[c].append([0]) #(Value)
    for k in range(1,len(p[c])):
        f[c][0].append(p[c][k][0])
        f[c][1].append(f[c][1][-1] + p[c][k-1][1])
    p[c] = dict(p[c])
    return p,f
...
def encode(x, b): #Text = x, Frequency Table Bias = b
    ...
    t = [[b for i in range(129)] for k in range(129)] #Empty probability table context 1
    for i in range(129): 
        t[i][128] = sum(t[i]) - t[i][128] #Laplacian Bias
    ...
    for k in range(len(x)): # for every symbol
        ...
        if k == 0:
            t[128][x[k]] += 1
            t[128][128] += 1
            p,f = enupdate(p,f,t,128)
        else:
            t[x[k-1]][x[k]] += 1
            t[x[k-1]][128] += 1
            p,f = enupdate(p,f,t,x[k-1])
...
def decode(y,b,n): #Coded Text = y, bias = b, number of characters in original text = n
    ...
    for k in range(n):
        ...
        if k == 0:
            t[128][x[k]] += 1
            t[128][128] += 1
            p,f = deupdate(p,f,t,128)
        else:
            t[x[k-1]][x[k]] += 1
            t[x[k-1]][128] += 1
            p,f = deupdate(p,f,t,x[k-1])
