...
def shannon_fano(p):
    ...
    # in Shannon's paper.
    p = sorted([(a,p[a]) for a in p if p[a]>0.0], key = lambda el: el[1], reverse = True)
...
def adhuffman(x,p):
    # for i in range(128):
    # p[i] = 1/128
    let = [i for i in range(128)] #initial character leaf indexes
    xt = huffman(p) #initial Tree
    w = calcweights(xt) #initial node weights
    y = []
    for k in range(len(x)):
        if k % 100 == 0:
            so.write('Adaptive Huffman encoded %d%%    \r' % int(floor(k/len(x)*100)))
            so.flush()
        y += codeword(xt,x[k])
        result = update(xt,x[k], w,let)
        xt = result[0]
        w = result[1]
        let = result[2]
    return y

def adhuffmandec(y,p):
    let = [i for i in range(128)] #initial character leaf indexes
    xt = huffman(p) #initial Tree
    w = calcweights(xt) #initial node weights
    x = []
    n = 0
    while n < len(y):
        mess = charfind(xt,y[n:])
        x.append(mess[0])
        n += mess[1]+1
        result = update(xt,mess[1], w,let)
        xt = result[0]
        w = result[1]
    return x
