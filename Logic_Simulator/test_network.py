...
def test_execute_siggen(new_network):
    """Test if execute_network returns the correct output for siggen."""
    network = new_network
    devices = network.devices
    names = devices.names
    ...
    # Make SIGGEN and an OR
    [SIG_ID, OR_ID] = names.lookup(["Sig1", "Or1"])

    # Make devices
    devices.make_device(SIG_ID, devices.SIGGEN, [0, 1, 1, 1])
    devices.make_device(OR_ID, devices.OR, [1])

    # Make connections
    network.make_connection(SIG_ID, None, OR_ID, I1)

    # Get device outputs
    sig_output = "network.get_output_signal(SIG_ID, None)"
    or_output = "network.get_output_signal(OR_ID, None)"

    # Ensure outputs are expected values
    assert [eval(sig_output), eval(or_output)] == [LOW, LOW]
    # Get next value input from siggen
    network.execute_network()
    network.execute_network()
    assert [eval(sig_output), eval(or_output)] == [HIGH, HIGH]
    network.execute_network()
    assert [eval(sig_output), eval(sig_output)] == [HIGH, HIGH]
    network.execute_network()
    assert [eval(sig_output), eval(sig_output)] == [HIGH, HIGH]
    network.execute_network()
    assert [eval(sig_output), eval(sig_output)] == [LOW, LOW]
