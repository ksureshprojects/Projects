...
class Device:
    ...
    def __init__(self, device_id):
        ...
        self.siggen_sequence = None
        self.siggen_sequence_len = None
        self.total_time = 0
...
class Devices:
    ...
    make_siggen(self, device_id, waveform): Makes a signal generator with the
                                            specified waveform.
    ...
    def __init__(self, names):
        ...
        [self.NO_ERROR, self.INVALID_QUALIFIER, self.NO_QUALIFIER,
         self.BAD_DEVICE, self.QUALIFIER_PRESENT, self.TOO_MANY_QUALIFIER,
         self.DEVICE_PRESENT,
         self.INVALID_QUALIFIER_LIST] = self.names.unique_error_codes(8)
    ...
    def set_clock(self, device_id, half_period):
        """Set the switch state of the specified device to signal.
        ...
        if device is None:
            ...
            device.clock_half_period = half_period
            return True
    ...
    def make_siggen(self, device_id, waveform):
        """Make a signal generator with the specified waveform.

        waveform is a list of the binary values for the signal."""
        self.add_device(device_id, self.SIGGEN)
        device = self.get_device(device_id)
        device.siggen_sequence = waveform
        device.siggen_sequence_len = len(waveform)
        self.add_output(device_id, output_id=None, signal=waveform[0])
        device.total_time = 1
        device.clock_counter = random.randrange(1)
        # The signal generator should update every 1 half period
    ...
    def make_device(self, device_id, device_kind, device_property=None):
        ...
        if self.get_device(device_id) is not None:
            ...
            if device_property is None:
                ...
                error_type = self.TOO_MANY_QUALIFIER
            ...
            # Device property is a list of binary values of 0 or 1
            if device_property is None:
                error_type = self.NO_QUALIFIER
            elif not(set(device_property) <= set([self.LOW, self.HIGH])):
                error_type = self.INVALID_QUALIFIER_LIST
            else:
            ...
            if device_kind == self.XOR:
                ...
                if device_property is None:
                    ...
                    if device_property[0] not in range(1, 17):  # between 1-16
                        error_type = self.INVALID_QUALIFIER
                    else:
                        self.make_gate(device_id, device_kind,
                                       device_property[0])
                        error_type = self.NO_ERROR
                else:
                    error_type = self.TOO_MANY_QUALIFIER
