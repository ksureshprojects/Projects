...
def test_find_devices(devices_with_items):
    ...
    device_names = [AND1_ID, NOR1_ID, SW1_ID,
                    SIGGEN_ID] = names.lookup(["And1", "Nor1", "Sw1",
                                               "Siggen1"])
    ...
    assert devices.find_devices(devices.SIGGEN) == [SIGGEN_ID]
...
@pytest.mark.parametrize("function_args, error", [
    ...
    ("(OR_ID, new_devices.OR, [1,0,1])", "new_devices.TOO_MANY_QUALIFIER"),
    ("(SIG_ID, new_devices.SIGGEN, [1,0,1])", "new_devices.NO_ERROR"),
...
def test_make_device_gives_errors(new_devices, function_args, error):
    ...
    names = new_devices.names
    [AND1_ID, SW1_ID, CL_ID, D_ID, X1_ID, OR_ID, SIG_ID,
     X2_ID] = names.lookup(["And1", "Sw1", "Clock1", "D1", "Xor1",
                            "Or1", "Sig1", "Xor2"])
