...
class Network:
    ...
    execute_clock(self, device_id): Simulates a signal generator and updates
                                    its output signal value.

    execute_clock(self, device_id): Simulates a clock and updates its output
                                    signal value.

    update_siggens(self): If it is time to do so, sets siggen signals to
                          RISING or FALLING.
    ...
    def update_siggens(self):
        ...
        for device_id in siggen_devices:
            device = self.devices.get_device(device_id)
            # The below should by default be true for a signal generator as
            # half-periof is fixed to 1. Leave check here for possibility of
            # future further maintenance
            if device.clock_counter == device.clock_half_period:
                device.clock_counter = 0
                output_signal = self.get_output_signal(device_id,
                                                       output_id=None)
                # Update signal to the next value in waveform
                # Return to first value if end of waveform reached
                next_sig = device.siggen_sequence[device.total_time %
                                                  device.siggen_sequence_len]
                device.total_time += 1
                if output_signal != next_sig:
