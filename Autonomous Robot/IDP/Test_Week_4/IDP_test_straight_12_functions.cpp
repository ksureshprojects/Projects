...
void straight_path()
{
    ...
    while(true)
    {
       ...
       /* if((IR | 248) == NONE) // Line Lost
        {
			rlink.command(MOTOR_1_GO, 0*SPEED); // Stop, line lost
            rlink.command(MOTOR_2_GO, 0*SPEED); 
            delay(500);
            rlink.command(MOTOR_1_GO, 0.95*SPEED); // reverse 
            rlink.command(MOTOR_2_GO, 0.95*SPEED + 128);
            while ((IR & 1) != 1) // until central sensor back on line
            {
                IR = rlink.request(READ_PORT_7);
				cout << "finding line" << endl;
            }
            rlink.command(MOTOR_2_GO, 0*SPEED); // Stop, line found
            rlink.command(MOTOR_2_GO, 0*SPEED); 
...
void pick_up()
{
	...
	balls_array[ballcount] = ball_type(weightswitch, ldr); //Store type of ball
...
int ball_type(int weightswitch, int ldr)
{
